/**
 * Bracket Auto-Closure Extension for Obsidian's Main Editor
 *
 * Provides intelligent bracket auto-closure and auto-removal for:
 * - {% %} and {%? ?%} prompt syntax
 * - () parentheses inside prompt syntax
 *
 * Note: {{ }} auto-closure is NOT enabled in the main editor (only in Settings Title formatter)
 */

import { EditorView, keymap } from "@codemirror/view";
import { Extension } from "@codemirror/state";
import { findCodeBlockRanges, isInsideCodeBlock } from "./PromptHighlighter";
 ...
 */
function createBracketInputHandler(): Extension {
  return EditorView.inputHandler.of((view, from, to, text) => {
    const doc = view.state.doc.toString();
    const textBefore = doc.slice(0, from);

    // Check if we're inside a code block - don't auto-close there
    const codeBlockRanges = findCodeBlockRanges(doc);
 ...
 */
function createBracketDeleteKeymap(): Extension {
  return keymap.of([
    {
      key: "Backspace",
      run: (view) => {
        ...
        // Check if we're inside a code block - don't auto-delete there
        const codeBlockRanges = findCodeBlockRanges(doc);
        if (isInsideCodeBlock(from, codeBlockRanges)) {
          return false;
        }
...
/**
 * Creates the bracket closure extension for the main Obsidian editor
 * @returns CodeMirror extension for bracket auto-closure
 */
export function createEditorBracketClosureExtension(): Extension {
  return [
    createBracketInputHandler(),
    createBracketDeleteKeymap(),
  ];
}
